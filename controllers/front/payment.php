...
class BOIPAPaymentModuleFrontController extends ModuleFrontController
{
    /** @var BOIPA */
    private $boipa;
    ...
    public function initContent()
    {
        ...
        if ($retry) {
            $this->boipa->checkAccess($retry, $retryToken);
        ...
        $token = $this->boipa->getPaymentToken($tokenEvo, $retry, $redirect, $amount);
    ...
    private function payByEvo($id_evo_payment, $status, $retry=0, $tokenEvo, $retryToken){
        ...
        if($this->boipa->getPaymentStatus($tokenEvo)->result!==$status){
            $status='failure';
        }
        ...
        if ((int)$retry !== 0) {
            ...
            $order = $this->boipa->createOrder($status);
        }
    ...
    private function getShowPayMethodsParameters($retry = '', $token, $merchantTxId)
    {
        ...
        if ($retry != '') {
            $evoPayment = $this->boipa->getOrdersByIdOrder($retry);
            $order = new Order($retry);
            ...
            return [
                ...
                'PayAction' => $this->context->link->getModuleLink('boipa', 'payment'),
