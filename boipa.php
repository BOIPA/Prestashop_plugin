...
include_once _PS_MODULE_DIR_ . '/boipa/tools/sdk/payments.php';
include_once _PS_MODULE_DIR_ . '/boipa/translations/statuses.php';

class BOIPA extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('BOIPA');
        $this->description = $this->l('Accepts payments by BOIPA');
    ...
    public function displayForm()
    {
        $form['boipa'] = [
            ...
            $form['merchant'] = [
                'form' => [
                    'legend' => [
                        'title' => $this->l('BOIPA Connection settings'),
                        'icon' => 'icon-cog'
                    ],
                    'input' => [
                        [
        ...
        $this->addIntegrationMode($form['boipa']['form']['input']);
    ...
    public function buildTemplatePath($name)
    {
        ...
        return 'module:boipa/views/templates/front/' . $name . '17.tpl';
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $paymentOption->setCallToActionText($this->l('Pay with BOIPA'))
...
//         $langCode = (new Language($order->id_lang)) -> iso_code;
        ...
        if (Configuration::get('EVO_REFUND_POSSIBILITY') && $order->module == 'boipa') {
            $lastStatus = 0;
            ...
            if ($orders) {
                foreach ($orders as &$orderr) {
                    ...
                    if($rawInfo !== ""){
                        ...
                        $orderr['info'] = $this->l('Refunded: ', 'boipa').$rawInfo;
                    }
    ...
    private function refundProcess($order, $tokenEvo)
    {
        ...
        if($possibleToRefund < 0 || round($refundAmount,2) > round($possibleToRefund,2)){
            return ['state' => 'danger', 'text' => $this->l('You can not make a refund on this amount for this order.', 'boipa')];
        }
        ...
        if(!$refund->errors){
            ...
            //$order->addOrderPayment('-'.$refundAmount, 'BOIPA', $id_evo_payment, $currency, date('Y-m-d H:i:s'));
            ...
            return ['state' => 'danger', 'text' => $this->l('No connect with payment gateway.', 'boipa')];
        }
        return false;
    }
    ...
    private function getSumCurrentOrderPayments($order)
    {
        ...
        foreach ($payments as $payment) {
            if($payment->payment_method=='BOIPA'){
    ...
    {
        $link = $this->context->link->getModuleLink('boipa', 'payment');
    ...
    {
        $paymentOptions = [
            'cta_text' => $this->l('Pay with BOIPA'),
            'action' => $this->context->link->getModuleLink('boipa', 'payment'),
    ...
    public function getPaymentToken($merchantTxId, $retry='', $mode, $amount='')
    {
        ...
        try {
            ...
            $notificationUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=boipa&controller=response&ipg=0&retry=' . $retry;
            $landingUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=boipa&controller=response&ipg=0&retry=' 
    ...
    /**
     * get payment status description by BOIPA status
     */
